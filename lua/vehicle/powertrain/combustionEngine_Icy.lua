...
local function updateTorque(device, dt)
  ...
  if device.idleAVTimer > 1/device.idleRPMVarianceFrequency then -- ZEIT EDIT
    if device.idleAVVariance > -device.idleRPMVariance*rpmToAV then -- ZEIT EDIT
      device.idleAVVariance = -device.idleRPMVariance*rpmToAV -- ZEIT EDIT
    else -- ZEIT EDIT
      device.idleAVVariance = device.idleRPMVariance*rpmToAV -- ZEIT EDIT
    end -- ZEIT EDIT
      device.idleAVTimer = 0 -- ZEIT EDIT
  end -- ZEIT EDIT

  local idleAV = max(device.idleAV, device.idleAVOverwrite) + device.idleRPMSmoother:get(device.idleAVVariance) -- ZEIT EDIT
...
local function reset(device, jbeamData)
  ...
  device.idleAVReadError = jbeamData.idleAVReadError or 0
  ...
  device.slowIgnitionErrorTimer = jbeamData.slowIgnitionErrorTimer or 0
  device.slowIgnitionErrorChance = jbeamData.slowIgnitionErrorChance or 0.0
  device.slowIgnitionErrorCoef = jbeamData.slowIgnitionErrorCoef or 1
  device.fastIgnitionErrorSmoother:reset()
  device.fastIgnitionErrorChance = jbeamData.fastIgnitionErrorChance or 0.0
  device.fastIgnitionErrorCoef = jbeamData.fastIgnitionErrorCoef or 1
...
local function new(jbeamData)
  local device = {
	...
	idleRPMVariance = jbeamData.idleRPMVariance or 3000, -- ZEIT EDIT
    idleRPMVarianceFrequency = jbeamData.idleRPMVarianceFrequency or 3, -- ZEIT EDIT
    idleRPMSmoother = newLinearSmoothing(physicsDt*1000), -- ZEIT EDIT
    idleAVTimer = 0, -- ZEIT EDIT
    idleAVVariance = 0, -- ZEIT EDIT
    --
...
local rawPart1Curve = {}
  local lastRawPart1Value = 0
  if jbeamData.torqueModPart1 then
    local Part1TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart1)
    local rawPart1Points = {}
    for _, v in pairs(Part1TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart1Points, {v.rpm, v.torque})
    end
    rawPart1Curve = createCurve(rawPart1Points)
    lastRawPart1Value = rawPart1Curve[#rawPart1Curve]
  end
  
  
  local rawPart2Curve = {}
  local lastRawPart2Value = 0
  if jbeamData.torqueModPart2 then
    local Part2TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart2)
    local rawPart2Points = {}
    for _, v in pairs(Part2TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart2Points, {v.rpm, v.torque})
    end
    rawPart2Curve = createCurve(rawPart2Points)
    lastRawPart2Value = rawPart2Curve[#rawPart2Curve]
  end
  
  local rawPart3Curve = {}
  local lastRawPart3Value = 0
  if jbeamData.torqueModPart3 then
    local Part3TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart3)
    local rawPart3Points = {}
    for _, v in pairs(Part3TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart3Points, {v.rpm, v.torque})
    end
    rawPart3Curve = createCurve(rawPart3Points)
    lastRawPart3Value = rawPart3Curve[#rawPart3Curve]
  end
  
  local rawPart4Curve = {}
  local lastRawPart4Value = 0
  if jbeamData.torqueModPart4 then
    local Part4TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart4)
    local rawPart4Points = {}
    for _, v in pairs(Part4TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart4Points, {v.rpm, v.torque})
    end
    rawPart4Curve = createCurve(rawPart4Points)
    lastRawPart4Value = rawPart4Curve[#rawPart4Curve]
  end
  
  local rawPart5Curve = {}
  local lastRawPart5Value = 0
  if jbeamData.torqueModPart5 then
    local Part5TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart5)
    local rawPart5Points = {}
    for _, v in pairs(Part5TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart5Points, {v.rpm, v.torque})
    end
    rawPart5Curve = createCurve(rawPart5Points)
    lastRawPart5Value = rawPart5Curve[#rawPart5Curve]
  end
  
  local rawPart6Curve = {}
  local lastRawPart6Value = 0
  if jbeamData.torqueModPart6 then
    local Part6TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart6)
    local rawPart6Points = {}
    for _, v in pairs(Part6TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart6Points, {v.rpm, v.torque})
    end
    rawPart6Curve = createCurve(rawPart6Points)
    lastRawPart6Value = rawPart6Curve[#rawPart6Curve]
  end
  
  local rawPart7Curve = {}
  local lastRawPart7Value = 0
  if jbeamData.torqueModPart7 then
    local Part7TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart7)
    local rawPart7Points = {}
    for _, v in pairs(Part7TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart7Points, {v.rpm, v.torque})
    end
    rawPart7Curve = createCurve(rawPart7Points)
    lastRawPart7Value = rawPart7Curve[#rawPart7Curve]
  end
  
  local rawPart8Curve = {}
  local lastRawPart8Value = 0
  if jbeamData.torqueModPart8 then
    local Part8TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart8)
    local rawPart8Points = {}
    for _, v in pairs(Part8TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart8Points, {v.rpm, v.torque})
    end
    rawPart8Curve = createCurve(rawPart8Points)
    lastRawPart8Value = rawPart8Curve[#rawPart8Curve]
  end
  
  local rawPart9Curve = {}
  local lastRawPart9Value = 0
  if jbeamData.torqueModPart9 then
    local Part9TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart9)
    local rawPart9Points = {}
    for _, v in pairs(Part9TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart9Points, {v.rpm, v.torque})
    end
    rawPart9Curve = createCurve(rawPart9Points)
    lastRawPart9Value = rawPart9Curve[#rawPart9Curve]
  end
  
  local rawPart10Curve = {}
  local lastRawPart10Value = 0
  if jbeamData.torqueModPart10 then
    local Part10TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart10)
    local rawPart10Points = {}
    for _, v in pairs(Part10TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart10Points, {v.rpm, v.torque})
    end
    rawPart10Curve = createCurve(rawPart10Points)
    lastRawPart10Value = rawPart10Curve[#rawPart10Curve]
  end
  
  local rawPart11Curve = {}
  local lastRawPart11Value = 0
  if jbeamData.torqueModPart11 then
    local Part11TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart11)
    local rawPart11Points = {}
    for _, v in pairs(Part11TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart11Points, {v.rpm, v.torque})
    end
    rawPart11Curve = createCurve(rawPart11Points)
    lastRawPart11Value = rawPart11Curve[#rawPart11Curve]
  end
  
  local rawPart12Curve = {}
  local lastRawPart12Value = 0
  if jbeamData.torqueModPart12 then
    local Part12TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart12)
    local rawPart12Points = {}
    for _, v in pairs(Part12TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart12Points, {v.rpm, v.torque})
    end
    rawPart12Curve = createCurve(rawPart12Points)
    lastRawPart12Value = rawPart12Curve[#rawPart12Curve]
  end
  
  local rawPart13Curve = {}
  local lastRawPart13Value = 0
  if jbeamData.torqueModPart13 then
    local Part13TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart13)
    local rawPart13Points = {}
    for _, v in pairs(Part13TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart13Points, {v.rpm, v.torque})
    end
    rawPart13Curve = createCurve(rawPart13Points)
    lastRawPart13Value = rawPart13Curve[#rawPart13Curve]
  end
  
  local rawPart14Curve = {}
  local lastRawPart14Value = 0
  if jbeamData.torqueModPart14 then
    local Part14TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart14)
    local rawPart14Points = {}
    for _, v in pairs(Part14TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart14Points, {v.rpm, v.torque})
    end
    rawPart14Curve = createCurve(rawPart14Points)
    lastRawPart14Value = rawPart14Curve[#rawPart14Curve]
  end
  
  local rawPart15Curve = {}
  local lastRawPart15Value = 0
  if jbeamData.torqueModPart15 then
    local Part15TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart15)
    local rawPart15Points = {}
    for _, v in pairs(Part15TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart15Points, {v.rpm, v.torque})
    end
    rawPart15Curve = createCurve(rawPart15Points)
    lastRawPart15Value = rawPart15Curve[#rawPart15Curve]
  end
  
  local rawPart16Curve = {}
  local lastRawPart16Value = 0
  if jbeamData.torqueModPart16 then
    local Part16TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart16)
    local rawPart16Points = {}
    for _, v in pairs(Part16TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart16Points, {v.rpm, v.torque})
    end
    rawPart16Curve = createCurve(rawPart16Points)
    lastRawPart16Value = rawPart16Curve[#rawPart16Curve]
  end
  
  local rawPart17Curve = {}
  local lastRawPart17Value = 0
  if jbeamData.torqueModPart17 then
    local Part17TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart17)
    local rawPart17Points = {}
    for _, v in pairs(Part17TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart17Points, {v.rpm, v.torque})
    end
    rawPart17Curve = createCurve(rawPart17Points)
    lastRawPart17Value = rawPart17Curve[#rawPart17Curve]
  end
  
  local rawPart18Curve = {}
  local lastRawPart18Value = 0
  if jbeamData.torqueModPart18 then
    local Part18TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart18)
    local rawPart18Points = {}
    for _, v in pairs(Part18TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart18Points, {v.rpm, v.torque})
    end
    rawPart18Curve = createCurve(rawPart18Points)
    lastRawPart18Value = rawPart18Curve[#rawPart18Curve]
  end
  
  local rawPart19Curve = {}
  local lastRawPart19Value = 0
  if jbeamData.torqueModPart19 then
    local Part19TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart19)
    local rawPart19Points = {}
    for _, v in pairs(Part19TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart19Points, {v.rpm, v.torque})
    end
    rawPart19Curve = createCurve(rawPart19Points)
    lastRawPart19Value = rawPart19Curve[#rawPart19Curve]
  end
  
  local rawPart20Curve = {}
  local lastRawPart20Value = 0
  if jbeamData.torqueModPart20 then
    local Part20TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart20)
    local rawPart20Points = {}
    for _, v in pairs(Part20TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart20Points, {v.rpm, v.torque})
    end
    rawPart20Curve = createCurve(rawPart20Points)
    lastRawPart20Value = rawPart20Curve[#rawPart20Curve]
  end
  
  local rawPart21Curve = {}
  local lastRawPart21Value = 0
  if jbeamData.torqueModPart21 then
    local Part21TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart21)
    local rawPart21Points = {}
    for _, v in pairs(Part21TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart21Points, {v.rpm, v.torque})
    end
    rawPart21Curve = createCurve(rawPart21Points)
    lastRawPart21Value = rawPart21Curve[#rawPart21Curve]
  end
  
  local rawPart22Curve = {}
  local lastRawPart22Value = 0
  if jbeamData.torqueModPart22 then
    local Part22TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart22)
    local rawPart22Points = {}
    for _, v in pairs(Part22TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart22Points, {v.rpm, v.torque})
    end
    rawPart22Curve = createCurve(rawPart22Points)
    lastRawPart22Value = rawPart22Curve[#rawPart22Curve]
  end
  
  local rawPart23Curve = {}
  local lastRawPart23Value = 0
  if jbeamData.torqueModPart23 then
    local Part23TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart23)
    local rawPart23Points = {}
    for _, v in pairs(Part23TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart23Points, {v.rpm, v.torque})
    end
    rawPart23Curve = createCurve(rawPart23Points)
    lastRawPart23Value = rawPart23Curve[#rawPart23Curve]
  end
  
    local rawPart24Curve = {}
  local lastRawPart24Value = 0
  if jbeamData.torqueModPart24 then
    local Part24TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart24)
    local rawPart24Points = {}
    for _, v in pairs(Part24TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart24Points, {v.rpm, v.torque})
    end
    rawPart24Curve = createCurve(rawPart24Points)
    lastRawPart24Value = rawPart24Curve[#rawPart24Curve]
  end
  
   local rawPart25Curve = {}
  local lastRawPart25Value = 0
  if jbeamData.torqueModPart25 then
    local Part25TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart25)
    local rawPart25Points = {}
    for _, v in pairs(Part25TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart25Points, {v.rpm, v.torque})
    end
    rawPart25Curve = createCurve(rawPart25Points)
    lastRawPart25Value = rawPart25Curve[#rawPart25Curve]
  end
  
   local rawPart26Curve = {}
  local lastRawPart26Value = 0
  if jbeamData.torqueModPart26 then
    local Part26TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart26)
    local rawPart26Points = {}
    for _, v in pairs(Part26TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart26Points, {v.rpm, v.torque})
    end
    rawPart26Curve = createCurve(rawPart26Points)
    lastRawPart26Value = rawPart26Curve[#rawPart26Curve]
  end
  
     local rawPart27Curve = {}
  local lastRawPart27Value = 0
  if jbeamData.torqueModPart27 then
    local Part27TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart27)
    local rawPart27Points = {}
    for _, v in pairs(Part27TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart26Points, {v.rpm, v.torque})
    end
    rawPart27Curve = createCurve(rawPart27Points)
    lastRawPart27Value = rawPart27Curve[#rawPart27Curve]
  end
  
  local rawPart28Curve = {}
  local lastRawPart28Value = 0
  if jbeamData.torqueModPart28 then
    local Part28TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart28)
    local rawPart28Points = {}
    for _, v in pairs(Part28TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart28Points, {v.rpm, v.torque})
    end
    rawPart28Curve = createCurve(rawPart28Points)
    lastRawPart28Value = rawPart28Curve[#rawPart28Curve]
  end
  
  local rawPart29Curve = {}
  local lastRawPart29Value = 0
  if jbeamData.torqueModPart29 then
    local Part29TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart29)
    local rawPart29Points = {}
    for _, v in pairs(Part29TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart29Points, {v.rpm, v.torque})
    end
    rawPart29Curve = createCurve(rawPart29Points)
    lastRawPart29Value = rawPart29Curve[#rawPart29Curve]
  end
  
 local rawPart30Curve = {}
  local lastRawPart30Value = 0
  if jbeamData.torqueModPart30 then
    local Part30TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart30)
    local rawPart30Points = {}
    for _, v in pairs(Part30TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart30Points, {v.rpm, v.torque})
    end
    rawPart30Curve = createCurve(rawPart30Points)
    lastRawPart30Value = rawPart30Curve[#rawPart30Curve]
  end

 local rawPart31Curve = {}
  local lastRawPart31Value = 0
  if jbeamData.torqueModPart31 then
    local Part31TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart31)
    local rawPart31Points = {}
    for _, v in pairs(Part31TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart31Points, {v.rpm, v.torque})
    end
    rawPart31Curve = createCurve(rawPart31Points)
    lastRawPart31Value = rawPart31Curve[#rawPart31Curve]
  end
  
  local rawPart32Curve = {}
  local lastRawPart32Value = 0
  if jbeamData.torqueModPart32 then
    local Part32TorqueTable = tableFromHeaderTable(jbeamData.torqueModPart32)
    local rawPart32Points = {}
    for _, v in pairs(Part32TorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawPart32Points, {v.rpm, v.torque})
    end
    rawPart32Curve = createCurve(rawPart32Points)
    lastRawPart32Value = rawPart32Curve[#rawPart32Curve]
  end
  ...
  for i = 0, maxAvailableRPM, 1 do
	...
	local Part1 = rawPart1Curve[i] or lastRawPart1Value
	local Part2 = rawPart2Curve[i] or lastRawPart2Value
	local Part3 = rawPart3Curve[i] or lastRawPart3Value
	local Part4 = rawPart4Curve[i] or lastRawPart4Value
	local Part5 = rawPart5Curve[i] or lastRawPart5Value
	local Part6 = rawPart6Curve[i] or lastRawPart6Value
	local Part7 = rawPart7Curve[i] or lastRawPart7Value
	local Part8 = rawPart8Curve[i] or lastRawPart8Value
	local Part9 = rawPart9Curve[i] or lastRawPart9Value
	local Part10 = rawPart10Curve[i] or lastRawPart10Value
	local Part11 = rawPart11Curve[i] or lastRawPart11Value
	local Part12 = rawPart12Curve[i] or lastRawPart12Value
	local Part13 = rawPart13Curve[i] or lastRawPart13Value
	local Part14 = rawPart14Curve[i] or lastRawPart14Value
	local Part15 = rawPart15Curve[i] or lastRawPart15Value
	local Part16 = rawPart16Curve[i] or lastRawPart16Value
	local Part17 = rawPart17Curve[i] or lastRawPart17Value
	local Part18 = rawPart18Curve[i] or lastRawPart18Value
	local Part19 = rawPart19Curve[i] or lastRawPart19Value
	local Part20 = rawPart20Curve[i] or lastRawPart20Value
	local Part21 = rawPart21Curve[i] or lastRawPart21Value
	local Part22 = rawPart22Curve[i] or lastRawPart22Value
	local Part23 = rawPart23Curve[i] or lastRawPart23Value
	local Part24 = rawPart24Curve[i] or lastRawPart24Value
	local Part25 = rawPart25Curve[i] or lastRawPart25Value
	local Part26 = rawPart26Curve[i] or lastRawPart26Value
	local Part27 = rawPart27Curve[i] or lastRawPart27Value
	local Part28 = rawPart28Curve[i] or lastRawPart28Value
    local exhaust = rawExhaustCurve[i] or lastRawExhaustValue
    rawCombinedCurve[i] = base * baseMult + intake + exhaust + Part1 + Part2 + Part3 + Part4 + Part5 + Part6 + Part7 + Part8 + Part9 + Part10 + Part11 + Part12 + Part13 + Part14 + Part15 + Part16 + Part17 + Part18 + Part19 + Part20 + Part21 + Part22 + Part23 + Part24 + Part25 + Part26 + Part27 + Part28
  end
